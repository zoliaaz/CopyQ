...
struct ScriptValueFactory {
    static QJSValue toScriptValue(const T &value, Scriptable *)
    {
        return QJSValue(value);
    }

    static T fromScriptValue(const QJSValue &value, Scriptable *)
    {
...
template <typename T>
QJSValue toScriptValue(const T &value, Scriptable *scriptable)
{
    return ScriptValueFactory<T>::toScriptValue(value, scriptable);
}

template <typename T>
T fromScriptValue(const QJSValue &value, Scriptable *scriptable)
{
    return ScriptValueFactory<T>::fromScriptValue(value, scriptable);
}

template <typename T>
void fromScriptValueDefined(const QJSValue &value, Scriptable *scriptable, T *outputValue)
{
...
struct ScriptValueFactory< QList<T> > {
    static QJSValue toScriptValue(const QList<T> &list, Scriptable *scriptable)
    {
    ...
    static QList<T> fromScriptValue(const QJSValue &value, Scriptable *scriptable)
    {
        if ( !value.isArray() )
            return QList<T>();
...
struct ScriptValueFactory<QVariantMap> {
    static QJSValue toScriptValue(const QVariantMap &dataMap, Scriptable *scriptable)
    {
    ...
    static QVariantMap fromScriptValue(const QJSValue &value, Scriptable *scriptable)
    {
        QVariantMap result;
...
struct ScriptValueFactory<QByteArray> {
    static QJSValue toScriptValue(const QByteArray &bytes, Scriptable *scriptable)
    {
...
struct ScriptValueFactory<QStringList> {
    static QJSValue toScriptValue(const QStringList &list, Scriptable *scriptable)
    {
        return ScriptValueFactory< QList<QString> >::toScriptValue(list, scriptable);
    }

    static QStringList fromScriptValue(const QJSValue &value, Scriptable *scriptable)
    {
...
struct ScriptValueFactory<QString> {
    static QJSValue toScriptValue(const QString &text, Scriptable *)
    {
        return QJSValue(text);
    }

    static QString fromScriptValue(const QJSValue &value, Scriptable *scriptable)
    {
...
struct ScriptValueFactory<QRegExp> {
    static QJSValue toScriptValue(const QRegExp &re, Scriptable *)
    {
        return QJSValue(re.pattern());
    }

    static QRegExp fromScriptValue(const QJSValue &value, Scriptable *scriptable)
    {
...
struct ScriptValueFactory<Command> {
    static QJSValue toScriptValue(const Command &command, Scriptable *scriptable)
    {
        QJSValue value = scriptable->engine()->newObject();
    ...
    static Command fromScriptValue(const QJSValue &value, Scriptable *scriptable)
    {
        Command command;
...
struct ScriptValueFactory<QVariant> {
    static QJSValue toScriptValue(const QVariant &variant, Scriptable *scriptable)
    {
        if ( !variant.isValid() )
    ...
    static QVariant fromScriptValue(const QJSValue &value, Scriptable *scriptable)
    {
...
QString availableCodecsMessage()
{
    QString codecs = "Available codecs are:";
    for (const auto &availableCodecName : QTextCodec::availableCodecs())
        codecs.append( "\n" + QString::fromUtf8(availableCodecName) );
    return codecs;
}

QTextCodec *codecFromName(const QJSValue &codecName, Scriptable *scriptable)
{
    return QTextCodec::codecForName( toByteArray(codecName, scriptable) );
}
...
bool Scriptable::toItemData(const QJSValue &value, const QString &mime, QVariantMap *data) const
{
    ...
    if ( value.isUndefined() )
        ...
        data->insert( mime, toByteArray(value, this) );
...
QJSValue Scriptable::toUnicode()
{
    ...
    if (argumentCount() >= 2) {
        const auto codec = codecFromName( argument(1), this );
        if (!codec)
            return throwError( availableCodecsMessage() );
...
QJSValue Scriptable::abort()
{
    ...
    if (m_engine) {
        m_aborted = true;
        return m_engine->evaluate("throw null");
    }
...
QVariant Scriptable::call(const QString &method, const QVariantList &arguments)
{
    ...
    const auto result = fn.call(fnArgs);
...
void Scriptable::executeArguments(const QByteArray &bytes)
{
    ...
    if ( args.isEmpty() ) {
        ...
        int skipArguments = 0;
        while ( skipArguments < fnArgs.size() && !m_aborted && !hasUncaughtException(result) ) {
        ...
        if ( !m_aborted && hasUncaughtException() ) {
            response = createScriptErrorMessage(m_uncaughtExceptionText).toUtf8();
...
QJSValue Scriptable::nextToClipboard(int where)
{
    ...
    return setClipboard(&data, QClipboard::Clipboard);
#ifdef HAS_MOUSE_SELECTIONS
...
bool Scriptable::hasUncaughtException(const QJSValue &value)
{
    ...
    m_uncaughtException = true;
    m_uncaughtExceptionText = value.toString();
    return true;
}
